...
defs['Session']=obj({'session_id':ID,'status':enum('open','finalized'),'revision':integer(),'started_at':TIME,'ended_at':nullable(TIME),'timezone':string(80),'facility':nullable(string(120)),'selection_required':BOOL,'duration_seconds':nullable(integer()),'duration_source':enum('user_reported','timestamps','unknown'),'overall_rpe':nullable({'type':'number','minimum':0,'maximum':10}),'notes':nullable(TEXT)},['session_id','status','revision','started_at','ended_at','timezone','facility','selection_required','duration_seconds','duration_source','overall_rpe','notes'])
...
tool('finalize_workout_session',['workout:write'],{**idem,'session_id':ID,'ended_at':TIME,'raw_text':TEXT,'duration_seconds':integer(),'overall_rpe':{'type':'number','minimum':0,'maximum':10},'notes':TEXT},['idempotency_key','expected_revision','session_id','ended_at','raw_text'],ref('Receipt'),True,True)
