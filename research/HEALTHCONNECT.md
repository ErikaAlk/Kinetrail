...
状态：**核实关卡 G-HC1–G-HC4 已通过；推送端点与手机 App 已实现并通过本地测试（分支 `feat/hc-reader`），未部署**。结论依据分三类标注：「静态分析」来自 FitDays+ 1.14.1 反编译代码；「官方文档」附链接；「未验证」必须在手机上核实后才能进入实现。
...
2. FitDays+ 只在**保存新称重时**、只为**账户主用户**写 HC：体重必写，体脂率、水分质量、骨量、基础代谢、去脂体重、心率在值大于 0 时写入。它不回填历史，也不会把 App 里的修改或删除同步到 HC（静态分析，见第 1 节）。
...
- **只写新保存的称重**，包括秤测和（推断）手动录入，不回填云端历史。
...
**两个来源都拿不到的**：肌肉率、骨骼肌率、蛋白质、皮下脂肪、内脏脂肪、身体年龄、BMI。旧 FitDays 的 157 条历史里有这些字段，今后的新数据不会再有。HC 只多出水分、骨量、BMR、去脂体重、心率；FitDays+ 云端只有 `weight_kg / bmi / pbf`，外加原始 `imps`。
...
- 称重在 App 里删掉，HC 里还在（G-HC4 已实测确认）。要从 Kinetrail 去掉误测，只能在系统 HC 界面删除，由推送带上删除（见 3.4）。
...
## 3. Kinetrail 架构变更（已实现，2026-09-15，分支 `feat/hc-reader`，未部署）
...
- 16:54:45 那组测试称重仍在手机 HC 里：在 HC 界面删除它的体重记录后，下一次同步会把它写成 tombstone；若首次同步前删掉，则根本不会入库。
...
用户断网称重一次（16:54:45 +08:00），读变更，联网让 FitDays+ 同步后再读，在 FitDays+ 里删除这条测试记录后再读。变更基线建立于 15:14。

| 问题 | 结果 | 依据 |
| --- | --- | --- |
| 什么时候写入 HC | 称重当下：6 条记录的 `last_modified` 都是 16:54:45.799，比测量时间晚不到 1 秒，不依赖联网 | 基线后的第一份变更文件含这 6 条 upsert |
| 联网补传会不会重复写 | 不会：之后 4 次读变更均为 0 条 upsert；18:56 读最近 30 天，每个时刻恰好一组 6 条 | 变更文件、30 天读取按 `time` 分组 |
| App 内删除会不会同步 | 不会：删除后读变更 0 条 deletion，与用户在 HC 界面看到的一致 | 变更文件 |
| 分组内 `last_modified` | 这次 6 条完全相同；14:22 那组相差 1 毫秒 | 仍不能用它分组 |

对设计的影响：

1. **重复规则可以保持简单**。实测没有补传重复，3.4 里“同一时刻同类型多条”的处理保留为防御，不需要额外去重逻辑。
2. **FitDays+ 里删掉的误测记录会一直留在 HC 里**，推送上去就会进入 Kinetrail。唯一的修正途径是用户在系统 HC 界面删除该记录，这会产生明确的 `DeletionChange`。原设计 V1 不接受删除，现在需要改为：只接受已入库 `hc_id` 的删除，写成 `is_deleted=1` 的新版本（旧版本保留、可恢复），不做物理删除。代价是令牌泄漏时攻击者能隐藏记录，但能从版本历史恢复。**用户已确认（2026-09-15），规则见 3.4。**
3. **16:54:45 那组测试记录现在还在手机 HC 里**（FitDays+ 已删）。推送上线前要在 HC 界面删掉它，或把 `HC_ACCEPT_AFTER` 设在它之后；14:22 那组是正式称重，应当保留。
