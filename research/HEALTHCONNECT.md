...
- 形近字：体→休（身休得分、目标休重）、控→挖（体重挖制）、肉→內 或 内（肌內均衝、肌内型）、衡→衝。解析时先把分不清的字按组归一（`CONFUSABLE`，含读成繁体的：龄/齡、级/級、测/測），四字以上标签再容忍一处编辑距离（同一分区的标签彼此至少差两个字；「分段脂肪分析」与「肌肉脂肪分析」正好差两个，阈值不能放宽到 2）。
- 表格边线读成行首「|」（|其它指标、|年龄:19）。
...
### 真机实跑（2026-09-16，一加 13 / ColorOS）

第一次拿 FitDays+ 分享出来的真实报告（2480×3508 PNG，`Pictures/Fitdays+/`）跑，核对页报「没认出年龄 / 身体得分 / 分段脂肪的五个部位」。调试版导出的 `files/last-ocr.json` 显示真实读法与近似图差得更多，已把它去掉 ID 后存为 `android/app/src/test/resources/mlkit-real-report-ocr.json` 当回归夹具：

| 报告上 | ML Kit 读成 | 处理 |
| --- | --- | --- |
| 年龄:19 | `年齡:19`（繁体） | 正则改为 `年[龄齡]`，并允许标签与数字之间有一两个杂字符 |
| 78/100分 | `781o0分`（`/1`→`1`、`0`→`o`） | 得分只认最高那行开头的数字，数字前缀改为最短匹配，否则会读成 781 |
| 肌肉均衡 | `肌肉內均衝`（多认一个字） | 标签比较从「等长且差一个字」放宽到编辑距离 ≤ 1 |
| 生物电阻抗 / 身体咸分分析 / 內脏脂肪等级 | `生物电阻坑` 等 | 同上，一处形近字 |
| 157.7% | `1577%`（丢小数点） | 见下 |
| 47.9 | `479`（丢小数点） | 见下 |

**丢小数点按固定位数还原。** FitDays+ 打印这些读数的 `ICERUnitConfig.o()` 先把值乘 10 取整，再在末位前插小数点，所以质量、比例、分段、阻抗一律一位小数，成分表里的体重是两位（`c.U`），内脏脂肪等级、基础代谢率、身体年龄本来就是整数。解析按字段的已知位数还原，不做通用猜测；整数字段不还原。

还原不了的那类错（位数对、数值错）仍由报告内部的算术关系拦下，核对页不给上传。「分析和建议」这个标题真机读成 `分祈和利建议`（两处错），匹配不上，只影响阻抗区的下界，回退到页面底部后结果不变。

**读不到不等于读错（2026-09-19 改）。** 每天的报告错的地方都不一样，一处容错总有被两处叠加突破的时候（这天真机把「内脏脂肪等级」读成「內脏脂肪等級」、「身体年龄」读成「身体休年齡」）。原来缺任何一个字段都整张作废，用户每天被卡住等改代码。现在必需的只有检测时间和体重（服务端就靠这两项挂到那次称重上，`REPORT_SCHEMA` 其余字段本来就是可选），别的指标没认出就整项不发，核对页列出缺项并写明补不回来。交叉校验不过照旧拦下——那是读错了。

修完后 22 项安卓单测全绿，其中两项直接跑这两份真实识别结果。真机上用同一张报告复测仍待用户操作（解锁手机后走一遍相册选图）。
