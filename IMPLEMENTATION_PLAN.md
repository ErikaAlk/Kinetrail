...
| 4 真实 CN 与平台容量（G1/G2） | G1 进行中：真实全量已发布、无阻断；`report_list` 确认为空数组；已改为只存本人；关联规则与分窗覆盖未核实；G2 未开始 | `docs/operations.md` 第 3 节与验证记录 |
| 5 “减肥计划”两聊天（G4/G5） | 未开始 | 需要部署后的 endpoint 与项目链接 |
| 6 部署交付 | 2026-09-14 已部署到自定义域名，匿名冒烟通过；本人绑定与 FitDays secrets 已设置 | `docs/operations.md`、`scripts/backup.mjs`/`restore.mjs`/`verify-restore.sql`（本地演练通过）、`THIRD_PARTY_NOTICES.md` |
