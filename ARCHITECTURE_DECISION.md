...
2. **未知上游数据集在 manifest 中记录结构。** 首次真实全量因每个窗口都有未登记的 `report_list` 而 partial，但原实现只记名字，无法按 DATA_CONTRACT 第 3 条分类。现记录形态、条数、键名和类型（可疑键名脱敏，不记值）。记录后确认 `report_list` 在全部 18 个窗口都是空数组，因此空的未知数据集不再标 partial。
3. **只存本人数据（用户决定）。** 同一 FitDays 账户下有 6 个成员，用户要求只保留自己并删除其他人。新增 `PROFILE_ALLOWLIST`，在消毒前丢弃其他成员与无 suid 的记录及昵称。已入库的 108 条其他成员记录（含 6 条 `p_unknown`，归属无法证明）与 5 个昵称，由 `scripts/purge-non-owner-profiles.sql` 临时移除禁止删除触发器后物理删除，再恢复触发器并验证拒绝删除。这是“事实表不物理删除”的一次性例外；D1 Time Travel 保留期内仍有副本。
4. **关闭周期同步（用户决定，计划迁移到 FitDays+）。** 用户反馈每次同步都把手机 App 顶下线。实验证实 FitDays 与 FitDays+ 都是同一账号只保留最后一次登录的 token，更换 client_id 或 os_type 都无效（`research/FITDAYSPLUS.md`）。新增 `PERIODIC_SYNC`，线上为 `off`：调度仍回收中断任务、执行 `refresh_data` 排队的任务，但不再每 6 小时自主登录。代价是读工具的 `stale` 基本常为 true，项目指令要求只在用户明确要求时刷新。用户随后决定迁移到 FitDays+；已静态分析 FitDays+ 1.14.1 还原登录与读取请求并用测试账号验证，适配器待迁移后按真实数据结构实现，迁移前 D1 书签记录在运维手册。
5. **体测改为 Health Connect 推送（用户决定，2026-09-15）。** 顶号无法绕过，FitDays+ 云端又只存 weight/bmi/pbf。静态分析 FitDays+ 1.14.1 并在用户手机上实测（G-HC1–G-HC4，`research/HEALTHCONNECT.md`）：Google 渠道包在保存主用户称重时写入 HC 6–7 个类型，不回填、不同步 App 内删除。新增只写端点 `POST /ingest/health-connect`（设备令牌、严格 schema、已存值不可变、HC 删除写 tombstone），复用暂存/发布/lease；手机端为自写最小 App（`android/`，打开即同步，无后台任务）。`refresh_data` 下线，stale 阈值 36 小时；FitDays 拉取代码保留但无入口。`wip/fitdaysplus-adapter` 不合并。未改变 D1 表结构。
6. **体脂秤网关与手机上物理删除称重（用户决定，2026-09-22）。** 用户要求上秤不用手机、在手机上管理并物理删除称重。沃莱 P3 的 BLE 协议与 WLA37 算法此前已逆出并对账（`research/P3.md`），常驻主机上的网关（`gateway/`，Python + bleak，systemd）只读连秤、算体成分，推送到新入口 `POST /ingest/scale`（独立令牌；体重与阻抗由服务端从 A7 帧自己解；身份按帧内容哈希）。A7 没有用户标识，用户同意把「只存本人」对这个来源放宽为体重窗口筛查 + 手机删除（DATA_CONTRACT 第 10 节）。`SCALE_ACCEPT_AFTER` 同时是 HC 来源的终点，两个来源不重叠。物理删除走 `POST /app/measurements/delete`（手机令牌再次扩权），迁移 0003 把两个禁止删除触发器改为只放行授权表里的记录，并新增墓碑挡住重推；只开放 `hc:`/`ble:` 来源（DATA_CONTRACT 第 11 节）。Astra（gpt-6-astra）实现前复核了设计，采纳：删除只开放新来源、帧内容做身份、按时间切换来源、暂存每批验 lease、调度不重排网关批次、帧解码校验声明长度、同步更新恢复校验与旧 purge 脚本；未采纳：删除时使旧 cursor 失效（改为在契约里写明被删记录会缺失）。MCP 工具定义未变，不需要在 ChatGPT 里重建连接。
