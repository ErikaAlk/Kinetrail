...
3. **只存本人数据（用户决定）。** 同一 FitDays 账户下有 6 个成员，用户要求只保留自己（Erika）并删除其他人。新增 `PROFILE_ALLOWLIST`，在消毒前丢弃其他成员与无 suid 的记录及昵称。已入库的 108 条其他成员记录（含 6 条 `p_unknown`，归属无法证明）与 5 个昵称，由 `scripts/purge-non-owner-profiles.sql` 临时移除禁止删除触发器后物理删除，再恢复触发器并验证拒绝删除。这是“事实表不物理删除”的一次性例外；D1 Time Travel 保留期内仍有副本。
