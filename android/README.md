...
- 日期数字下面是标记行：数字是当天训练会话的 `calories_kcal` 之和——手表的消耗要由模型在结束训练时写进 Kinetrail（用户把手表截图给模型），没写就只留一个实心点表示当天有训练；空心方块表示当天称过体重。标记行空着也占位，同一周的日期数字对齐。月历下有图例。今天是描边，选中是实色底。
- 点日期看当天，再点一次收起。每次训练一张卡片：抬头是时间段，下面是时长、消耗、RPE 和场馆；动作之间有分隔线，相同负重的连续组合并成「45 kg × 12、12、10」，负重变了另起一行；动作备注跟着动作，整场的备注单独放在卡片末尾。每次称重一张卡片：体重大字，体脂率和 BMI 常驻，其余指标（含报告补的内脏脂肪、骨骼肌这些）点「展开指标」后按脂肪、肌肉与骨骼、水分与蛋白质、其他读数分组显示，系统字号放大时改成单列。
...
一级导航是全局 4.6 的悬浮 Tab Bar：记录（训练日历）、同步（Health Connect 与报告识图）、设置（令牌与版本）。核对报告和推送令牌是从所属 tab 进去的子页面，返回键和顶栏返回都回到那个 tab，底栏仍高亮它。底栏材质的厚度是登记在案的覆盖：日历选中的日期格和详情页主按钮是 `accent.primary` 填充，会滚到条底下，全局默认的 0.48 那一档标签只有 2.6:1，收到浅 0.86 / 深 0.80（理由与实测值见 `KtMaterial`，`KtMaterialContrastTest` 钉住）。
