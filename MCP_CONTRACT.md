...
```text
Kinetrail（身迹）是我的体测与训练事实数据库，库里只有我一个人的数据，查询时不需要指定成员。
...
- 2026-09-15 之后的体测来自 Health Connect，只有体重、体脂率、骨量、基础代谢、水分等，BMI 按我的身高推算（标 bmi_derived_from_height）。我在手机上上传过报告的那次称重标 report_attached，额外有肌肉率、骨骼肌率、蛋白质、皮下脂肪、内脏脂肪、身体年龄、SMI、腰臀比，分段脂肪/肌肉与阻抗在完整记录 raw_json 的 report 里。没有这些指标的称重不要当作 0；比较新旧数据时只比两边都有的指标。
